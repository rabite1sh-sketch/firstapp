...
class WorkListView(ft.Column):
    ...
    def reload_works(self) -> None:
        """DB 작품 데이터를 조회해 카드 리스트를 갱신합니다."""
        works = self._work_service.get_works()

        if not works:
            self._work_list.controls = [
                ft.Card(content=ft.Container(padding=12, content=ft.Text("등록된 작품이 없습니다.")))
            ]
            return
                ...
                ft.Card(
                    content=ft.Container(
                        ...
                        content=ft.Column(
                            spacing=6,
                            controls=[
                                ft.Text(work["title"], size=18, weight=ft.FontWeight.W_600),
